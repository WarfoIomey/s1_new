...
/* структура, передаваемая в качестве аргумента обратным вызовам tcp */
struct echoclient{
	enum echoclient_states state; /* статус подключения */
	struct tcp_pcb *pcb;          /* указатель текущего тср соединения*/
	struct pbuf *p_tx;            /*указатель на pbuf, подлежащий передаче */
};



/*объявление вызовов функций*/
...
/**
  * @brief  Эхо соединение с ТСР сервером
  * @param  None
  * @retval None
  */
